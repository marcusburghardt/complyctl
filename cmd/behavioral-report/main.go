...
func main() {
	...
	evalLog := gemara.EvaluationLog{
		...
		Result:      aggregateResult(controlEvals),
	...
	// SARIF output excludes Passed evaluations via WithExcludedStatuses
	// to avoid noise in GitHub Code Scanning. Passed controls are still
	// recorded in the full EvaluationLog YAML for audit completeness.
	// Note: NotRun and NotApplicable are always skipped by gemaraconv.ToSARIF.
...
func writeSARIF(evalLog gemara.EvaluationLog, artifactURI, outDir string, catalog *gemara.ControlCatalog) error {
	sarifBytes, err := gemaraconv.ToSARIF(evalLog, gemaraconv.WithArtifactURI(artifactURI), gemaraconv.WithCatalog(catalog), gemaraconv.WithExcludedStatuses(gemara.Passed))
	if err != nil {
...
// aggregateResult computes the aggregate EvaluationLog result across all
// control evaluations using gemara.UpdateAggregateResult.
func aggregateResult(evals []*gemara.ControlEvaluation) gemara.Result {
	result := gemara.NotRun
	for _, e := range evals {
		if e == nil {
			continue
		}
		result = gemara.UpdateAggregateResult(result, e.Result)
	}
	if len(evals) == 0 {
