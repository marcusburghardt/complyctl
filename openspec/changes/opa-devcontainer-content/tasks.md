...
- [x] 1.1 Create `cmd/mock-oci-registry/testdata/test-opa-catalog.yaml` with a minimal OPA-appropriate control catalog (e.g., workstation audit or container security controls with OPA-evaluable assessment requirements)
- [x] 1.2 Create `cmd/mock-oci-registry/testdata/test-opa-policy.yaml` with `executor.id: opa` and assessment plans referencing the catalog requirements
- [x] 1.3 Add `seedDefaults()` entry to seed `policies/test-opa-policy` with the OPA catalog and policy files, tagged `v1.0.0` and `latest`
...
- [x] 4.1 Update `.devcontainer/scripts/post-create.sh` to copy any OPA granular policies into the test workspace (if required by the OPA provider's Generate/Scan flow)
- [x] 4.2 Verify the existing post-create.sh workspace setup copies the updated `complytime.yaml` with OPA entries

## 5. Documentation

- [x] 5.1 Update `docs/TESTING_ENVIRONMENT.md` Command Reference section to include OPA provider commands alongside Ampel examples
- [x] 5.2 Update `AGENTS.md` Recent Changes with the OPA devcontainer content entry
